...
const DetectionResults = ({ result }: DetectionResultsProps) => {
  ...
  const downloadPDFReport = () => {
    ...
    // Install jsPDF if not already available
  ...
  const generatePDF = () => {
    ...
    // @ts-ignore - jsPDF will be loaded dynamically
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF();
    
    // Set up the PDF
    doc.setFontSize(20);
    doc.text('Truth Nexus Guard', 20, 20);
    doc.setFontSize(16);
    doc.text('Deepfake Analysis Report', 20, 30);
    ...
    doc.setFontSize(10);
    doc.text(`File Name: ${result.fileName || 'N/A'}`, 20, 65);
    doc.text(`File Type: ${result.fileType || 'N/A'}`, 20, 75);
    doc.text(`Processing Time: ${result.processingTime}ms`, 20, 85);
    
    // Analysis result
    doc.setFontSize(12);
    doc.text('Analysis Result:', 20, 105);
    doc.setFontSize(10);
    
    const status = result.isDeepfake ? 'POTENTIAL DEEPFAKE DETECTED' : 'CONTENT APPEARS AUTHENTIC';
    const statusColor = result.isDeepfake ? [255, 0, 0] : [0, 128, 0];
    doc.setTextColor(...statusColor);
    doc.text(`Status: ${status}`, 20, 115);
    doc.setTextColor(0, 0, 0);
    
    doc.text(`Confidence Level: ${result.confidence.toFixed(1)}%`, 20, 125);
    
    // Detailed scores
    doc.setFontSize(12);
    doc.text('Detailed Analysis Scores:', 20, 145);
    doc.setFontSize(10);
    
    let yPos = 155;
    Object.entries(result.analysis).forEach(([key, value]) => {
      doc.text(`${key.charAt(0).toUpperCase() + key.slice(1)}: ${value.score.toFixed(1)}% - ${value.status}`, 20, yPos);
      yPos += 10;
    });
    
    // AI Analysis
    doc.setFontSize(12);
    doc.text('AI Analysis Summary:', 20, yPos + 10);
    doc.setFontSize(9);
    
    const cleanText = cleanExplanationText(result.explanation);
    const lines = doc.splitTextToSize(cleanText, 170);
    doc.text(lines, 20, yPos + 20);
    
    // Technical details (next page if needed)
    if (yPos + 80 > 280) {
      doc.addPage();
      yPos = 20;
    } else {
    ...
    doc.setFontSize(10);
    doc.text(`Model Version: v2.1.4`, 20, yPos + 10);
    doc.text(`Analysis Algorithm: Multi-modal CNN`, 20, yPos + 20);
    doc.text(`Risk Level: ${result.isDeepfake ? 'High' : 'Low'}`, 20, yPos + 30);
    doc.text(`Manipulation Probability: ${(100 - result.confidence).toFixed(1)}%`, 20, yPos + 40);
    
    // Disclaimer
    doc.setFontSize(8);
    doc.text('Disclaimer: This analysis is generated by AI and should be used as a preliminary assessment.', 20, 280);
